...
interface AntspaceHD5ContainerProps {
 ...
 */
export default function AntspaceHD5Container({
  position,
  containerId,
  onSelect,
  isSelected = false,
  rotation = [0, 0, 0],
}: AntspaceHD5ContainerProps) {
  ...
  const coolingPanelMaterial = useMemo(() => new THREE.MeshStandardMaterial({
    color: '#1e3a5f',
    metalness: 0.6,
    roughness: 0.4,
  }), []);

  const ventGrilleMaterial = useMemo(() => new THREE.MeshStandardMaterial({
    color: '#2c3e50',
    metalness: 0.9,
    roughness: 0.2,
  }), []);
